...
def predict(X, parameters):
    """
    makes a predictions for one given example using the given parameters of the model
    
    Arguments:
    X -- The example to be predicted, of shape (input size = 12288, number of training examples = 1)
    parameters -- parameters learnt by the model.

    
    Returns:
    a number 0-5 containing the prediction for the given example
    """
...
plt.figure(3)
plt.imshow(image)
plt.title("Your algorithm predicts: y = " + str(np.squeeze(my_image_prediction)))
plt.show()
